...
class EditCourse extends Component {
  ...
  getChapter = () => {
    ...
    axios.post(api.GET_CHAPTER, { course_id: this.props.route.params }, {
      'headers': {
      ...
      this.setState({
        ...this.state,
        chapters: res.data.response.chapters, username: res.data.response.user_name,
        domain:res.data.response.domain_sector,
        imagedata: res.data.response.course_image, courseName: res.data.response.course_name, courseData: res.data.response, isLoading: false,
        aboutCourse: res.data.response.course_description,
        imageBase: res.data.response.course_image,
        isAccount:res.data.response.course_type == 'free' ? true : false,
        price: res.data.response.course_price == undefined ? '0' : res.data.response.course_price,
        isActive: res.data.response.course_status == undefined ? false : res.data.response.course_status == 'private' ? true : false,
      });
    ...
    if (isCamera) {
      ...
      launchImageLibrary(
        ...
        (res) => {
          ...
          this.setState({ ...this.state, imagedata: res.uri, imageBase: res.base64, isImage: true });
        },
      );
    }
  };
  takePicture = async () => {
    if (this.camera) {
      ...
      this.setState({ ...this.state, imagedata: datas.uri, imageBase: datas.base64, isImage: true,isCamera:false });
    }
  };
  ...
  save = () => {
    if (this.state.courseName == '') {
      ...
      if (this.state.price == '') {
        ...
        this.setState({ ...this.state, isLoading: true });
        console.log('vdbvfh>>', this.state.isActive ? 'private' : 'public')
        axios.post(api.EDIT_COURSE, {
          ...
          image_update: this.state.isImage,
          ...
          <View style={{ alignItems: 'center' }}>
            ...
            {/* <View style={style.courseTypeVw}>
              <View style={style.accountVw}>
                <TouchableOpacity  onPress={() => { this.setState({ ...this.state, isAccount: true }); }} style={[style.accountBtn, { backgroundColor: this.state.isAccount ? Colors.headerColor : null, borderWidth: 1 }]}>
                  <Text style={[style.accountTx, { color: this.state.isAccount ? Colors.White : Colors.Black }]}>FREE</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  ...
                  style={[style.accountBtn, { backgroundColor: !this.state.isAccount ? Colors.headerColor : null, borderWidth: 1 }]}>
                  <Text style={[style.accountTx, { color: !this.state.isAccount ? Colors.White : Colors.Black }]}>PAID</Text>
                </TouchableOpacity>
              </View>
            </View> */}

            {/* {!this.state.isAccount ? <CustomInputView
              title={'Price'}
